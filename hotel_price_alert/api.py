...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def _handle_test_notify(self, payload: Dict[str, Any]) -> None:
        ...
        if not webhook:
            self._send_json({'error': '请先填写通知 Webhook'}, HTTPStatus.BAD_REQUEST)
            return
        try:
            send_test_notification(webhook, notify_type=notify_type)
            self._send_json({'ok': True})
        except ssl.SSLCertVerificationError:
            self._send_json({'error': '本机 Python 证书校验失败，请使用稳定版启动器重启服务后重试。'}, HTTPStatus.BAD_REQUEST)
        except Exception as exc:
            self._send_json({'error': f'测试通知发送失败: {exc}'}, HTTPStatus.BAD_REQUEST)
